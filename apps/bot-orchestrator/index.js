...
async function processProvisioningQueue() {
  ...
  try {
    ...
    // --- 2. Set Permissions (Linux only, skip on Windows) ---
    try {
      if (process.platform !== 'win32') {
        ...
        console.log(`[${instanceId}] ✓ Skipping chmod on Windows (not needed for Docker)`);
      }
    ...
    if (enhanced && (riskTemplate || customRiskConfig)) {
      console.log(`[${instanceId}] ✓ Applying enhanced risk configuration via UniversalRiskManager...`);
      try {
        // Map risk templates to riskLevel values for the new unified settings approach
        const riskLevelMapping = {
          conservative: 25,
          balanced: 50,
          aggressive: 75,
          dcaFocused: 60,
          portfolioRebalancing: 50
        };

        // Determine appropriate risk level based on template or custom config
        let riskLevel = 50; // default balanced
        let dcaEnabled = true;
        let autoRebalance = true;

        if (riskTemplate && riskLevelMapping[riskTemplate]) {
          riskLevel = riskLevelMapping[riskTemplate];
          dcaEnabled = riskTemplate === 'dcaFocused' || riskTemplate !== 'portfolioRebalancing';
          autoRebalance = riskTemplate === 'portfolioRebalancing';
          console.log(`[${instanceId}] Using template '${riskTemplate}' with riskLevel: ${riskLevel}`);
        } else if (customRiskConfig) {
          // Try to infer riskLevel from custom config's riskPerTrade
          const rpt = customRiskConfig.riskPerTrade || 0.02;
          riskLevel = Math.min(100, Math.max(0, Math.floor((rpt - 0.01) / 0.02 * 100)));
          dcaEnabled = customRiskConfig.dca?.enabled ?? true;
          autoRebalance = customRiskConfig.rebalancing?.enabled ?? false;
          console.log(`[${instanceId}] Inferred riskLevel ${riskLevel} from custom config`);
        }

        // Create UniversalRiskManager and apply settings (this will be saved to bot-settings.json)
        const riskManager = new UniversalRiskManager(instanceId, userId);
        await riskManager.updateSettings({
          enabled: true,
          riskLevel: riskLevel,
        ...
        console.log(`[${instanceId}] ✓ Risk configuration applied via UniversalRiskManager: riskLevel=${riskLevel}`);
      } catch (riskErr) {
    ...
    try {
      ...
      if (enhanced) {
        ...
        // Use default settings for standard bots (loadSettings creates defaults if not exists)
    ...
    console.log(`[${instanceId}] STEP 4.2: Initializing Universal Features (v2.0)...`);
    // --- 4.2. Initialize Universal Features for new bot ---
    try {
      const universalFeatures = new UniversalFeatures(instanceId, userId);
      
      // Configure features based on risk template or enhanced mode
      const featureConfig = {};
      
      if (enhanced) {
        // Enable more advanced features for enhanced bots
        featureConfig.takeProfitLevels = {
          enabled: true,
          mode: 'ladder',
          levels: [
        ...
        featureConfig.dailyLossProtection = {
          enabled: true,
        ...
        featureConfig.positionLimits = {
          enabled: true,
          maxPercentPerAsset: 30,
          maxPositionsPerAsset: 3,
          minTimeBetweenSameAsset: 3600,
          maxCorrelatedPositions: 2
        };
        featureConfig.volatilityAdjustment = {
          enabled: true,
          method: 'ATR',
          lookbackPeriod: 14,
          scaleFactor: 1.0,
        ...
        console.log(`[${instanceId}] Configuring enhanced universal features...`);
      }
      
      // Apply template-specific features
      if (riskTemplate === 'dcaFocused') {
        featureConfig.takeProfitLevels = {
          enabled: true,
          mode: 'ladder',
          levels: [
            { percentage: 3, exitPercent: 33 },
            { percentage: 6, exitPercent: 33 },
            { percentage: 12, exitPercent: 100 }
          ]
        };
        featureConfig.trailingStop = { enabled: false }; // DCA uses fixed TPs
      } else if (riskTemplate === 'conservative') {
        featureConfig.dailyLossProtection = {
          enabled: true,
          maxDailyLossPercent: 3, // Tighter loss limit
          pauseUntil: 'nextDay'
        };
        featureConfig.positionLimits = {
          enabled: true,
          maxPercentPerAsset: 20, // More conservative
          maxPositionsPerAsset: 2
        };
      }
      
      await universalFeatures.updateFeatures(featureConfig);
      console.log(`[${instanceId}] ✓ Universal Features v2.0 initialized`);
      
      // Register bot with Active Trade Monitor if running
      try {
        const monitor = getMonitor();
        if (monitor && monitor.running) {
          await monitor.registerBot(userId, instanceId, instanceDir);
          console.log(`[${instanceId}] ✓ Registered with Active Trade Monitor`);
        }
      } catch (monitorErr) {
        console.log(`[${instanceId}] Active Trade Monitor not available (non-critical): ${monitorErr.message}`);
      }
      
      console.log(`[${instanceId}] STEP 4.2 COMPLETE: Universal Features initialized.`);
    } catch (featuresErr) {
      console.warn(`[${instanceId}] Failed to initialize universal features: ${featuresErr.message}`);
    }
    ...
    try {
      ...
      // Set proper permissions (readable/writable by all) - Linux only
      if (process.platform !== 'win32') {
        ...
        console.log(`[${instanceId}] ✓ SQLite database file created (Windows mode)`);
      }
    } catch (dbError) {
...
`;
    ...
    // STEP 9: Initialize Universal Stake Override for Strategy-Independent Risk Management
    console.log(`[${instanceId}] STEP 9: Initializing universal stake override...`);
    try {
      await universalStakeOverride.initializeForBot(instanceId, userId, instanceDir);
      console.log(`[${instanceId}] ✅ Universal stake override initialized successfully`);
    } catch (stakeError) {
      console.error(`[${instanceId}] Failed to initialize universal stake override:`, stakeError.message);
      console.log(`[${instanceId}] Bot will continue without universal risk management`);
    }
    console.log(`[${instanceId}] STEP 9 COMPLETE: Universal stake override setup finished.`);
